"""
scripts/23_train_exp20_mixed_split.py
=======================================
Experiment 20: LightGBM + Extended Lag Features + Optuna + Station Category
DATA SPLIT: Mixed (Random Shuffle) instead of Temporal.

Goal: Evaluate interpolation performance vs extrapolation performance.
Includes combined [Val + Test] metrics.
"""
...
# ---------------------------------------------------------------------------
# Feature engineering (from Exp 4)
# ---------------------------------------------------------------------------
...
def main():
    ...
    report = [f"EXPERIMENT 20: Mixed (Random) Split Evaluation\n", "="*60 + "\n"]
    ...
    for target in TARGETS:
        ...
        # Save model
        model.save_model(str(OUT_DIR / f"{target}_lgbm_mixed.txt"))
        ...
        report.append(f"  Val+Test:  R2={vt_m['R2']:.4f}  RMSE={vt_m['RMSE']:.2f} (MIXED HOLDOUT)\n\n")

    with open(OUT_DIR / "exp20_report.txt", "w") as f:
        f.writelines(report)
    
    log.info(f"Experiment 20 complete. Results in {OUT_DIR}")
