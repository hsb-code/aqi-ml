...
OUT_DIR  = Path("models/exp16")
TARGETS  = ["PM25", "PM10"]

TRAIN_END = "2023-06-30"
VAL_END   = "2023-12-31"

# We replace Station lags with DRP lags for "Generalization"
BASE_FEATURES = [
...
def main():
    ...
    for target in TARGETS:
        log.info(f"===== Training Exp 16: {target} (DRP + Advanced LUR) =====")
        
        # Features: Base + DRP lag for this target
        drp_col = f"DRP_{target}_lag1"
        features = BASE_FEATURES + [drp_col]
        ...
        # Train full model (Train + Val)
        dtrain_full = lgb.Dataset(pd.concat([X_train, X_val]), 
                                  label=np.concatenate([y_train, y_val]), 
                                  categorical_feature=cat_features)
        
        model = lgb.train(best_params, dtrain_full, num_boost_round=500)
        
        model_path = OUT_DIR / f"{target}_exp16_drp.txt"
    ...
    with open(OUT_DIR / "exp16_report.txt", "w") as f:
        f.write("EXPERIMENT 16: Dynamic Regional Persistence (DRP) & Advanced LUR\n")
        f.write("="*60 + "\n")
        for res in results:
            f.write(res + "\n")
            
    log.info(f"Report saved to {OUT_DIR}/exp16_report.txt")
